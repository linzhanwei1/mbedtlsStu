...
static int generic_check(const mbedtls_md_info_t *md_info, char *filename)
{
    ...
    if (nb_err1 != 0)
    {
        mbedtls_printf("WARNING: %d (out of %d) input files could not be read\n", nb_err1, nb_tot1);
    }
    if (nb_err2 != 0)
    {
        mbedtls_printf("WARNING: %d (out of %d) computed checksums did not match\n", nb_err2, nb_tot2);
    }
    fclose(f);
