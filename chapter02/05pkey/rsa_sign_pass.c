...
int main(int argc, char *argv[])
{
    ...
    if ((ret = mbedtls_pk_parse_keyfile(&pk, argv[1], "", mbedtls_ctr_drbg_random, &ctr_drbg)) != 0) {
        ...
        mbedtls_printf(" ! mbedtls_pk_parse_public_keyfile returned -0x%0x\n\n", (unsigned int)-ret);
        goto exit;
    }
    ...
    if ((ret = mbedtls_pk_sign(&pk, MBEDTLS_MD_SHA256, hash, 0, buf, sizeof(buf), &olen, mbedtls_ctr_drbg_random, &ctr_drbg)) != 0) {
