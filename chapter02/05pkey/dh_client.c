...
int main(int argc, char *argv[])
{
    ...
    /* Read the server's public RSA key */
    ...
    // 这里应该是BUG
    // if ((ret = mbedtls_rsa_pkcs1_verify(&rsa, MBEDTLS_MD_SHA256, 32, hash, p)) != 0) {
    if ((ret = mbedtls_rsa_pkcs1_verify(&rsa, MBEDTLS_MD_SHA1, 20, hash, p)) != 0) {
    ...
    n = mbedtls_dhm_get_len(&dhm);
    if ((ret = mbedtls_dhm_make_public(&dhm, (int)n, buf, n, mbedtls_ctr_drbg_random, &ctr_drbg)) != 0) {
