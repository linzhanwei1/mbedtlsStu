...
#if defined(MBEDTLS_PK_PARSE_C) && defined(MBEDTLS_PK_WRITE_C) && defined(MBEDTLS_FS_IO) && defined(MBEDTLS_ENTROPY_C) && defined(MBEDTLS_CTR_DRBG_C)
...
int main(int argc, char *argv[])
{
    ...
    if (opt.mode == MODE_PRIVATE) {
        ...
        ret = mbedtls_pk_parse_keyfile(&key, opt.filename, NULL, mbedtls_ctr_drbg_random, &ctr_drbg);
        if (ret != 0) {
