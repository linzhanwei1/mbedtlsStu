/* Enable definition of fileno() even when compiling with -std=c99.Must be set before mbedtls_config.h, which pulls in glibc's features.h indirectly.
...
#    include <stdlib.h>
#    include <string.h>
#endif /* defined(MBEDTLS_CIPHER_C) && defined(MBEDTLS_MS_C) && defined(MBEDTLS_FS_IO) */
...
#    endif
    ...
    if (mode == MODE_ENCRYPT) {
        /* Generate the initialization vector as: IV = MD(filesize || filename)[0...15] */
        for (i = 0; i < 8; i++)
        ...
        /* Append the IV at he beginning of the output. */
        if (fwrite(IV, 1, 16, fout) != 16) {
            mbedtls_fprintf(stderr, "fwrite(%d bytes) failed\n", 16);
            goto exit;
        }

        /* Hash the IV and the secret key together 8192 times using the result to setup the AES context and HMAC */
        memset(digest, 0, 32);
        memcpy(digest, IV, 16);
        ...
        if (mbedtls_cipher_setkey(&cipher_ctx, digest, cipher_info->MBEDTLS_PRIVATE(key_bitlen), MBEDTLS_ENCRYPT) != 0) {
            mbedtls_fprintf(stderr, "mbdtls_cipher_setkey() returned error\n");
            goto exit;
        }
        ...
        for (offset = 0; offset < filesize; offset += mbedtls_cipher_get_block_size(&cipher_ctx)) {
            ...
            if (mbedtls_cipher_finish(&cipher_ctx, output, &olen) != 0) {
                mbedtls_fprintf(stderr, "mbedtls_cipher_finish() returnd error\n");
                goto exit;
            }
            /* hamc */
            mbedtls_md_hmac_update(&md_ctx, output, olen);
    ...
    if (mode == MODE_DECRYPT) {
        ...
        /* Check the file size. */
        if (cipher_info->MBEDTLS_PRIVATE(mode) != MBEDTLS_MODE_GCM && ((filesize - mbedtls_md_get_size(md_info)) % mbedtls_cipher_get_block_size(&cipher_ctx)) != 0) {
        ...
        if (mbedtls_cipher_setkey(&cipher_ctx, digest, cipher_info->MBEDTLS_PRIVATE(key_bitlen), MBEDTLS_DECRYPT) != 0) {
...
exit:
    ...
    /* Zeroize all command line arguments to also cover the case when the user has missed or reordered some, in which case the key might not be in argv[6]. */
    for (i = 0; i < argc; i++)
