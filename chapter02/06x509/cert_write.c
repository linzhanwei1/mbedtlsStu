...
int write_certificate(mbedtls_x509write_cert *crt, const char *output_file, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    int ret;
    FILE *f;
...
int main(int argc, char *argv[])
{
    ...
    opt.issuer_crt = DFL_ISSUER_CRT;           // 颁发者为空，默认空
    opt.request_file = DFL_REQUEST_FILE;       // 请求文件，默认为空
    opt.subject_key = DFL_SUBJECT_KEY;         // 证书使用者密钥 默认subject.key
    opt.issuer_key = DFL_ISSUER_KEY;           // 颁发者密钥，默认ca_privkey.pem
    opt.subject_pwd = DFL_SUBJECT_PWD;         // 证书使用者密码，默认空
    opt.issuer_pwd = DFL_ISSUER_PWD;           // 颁发者密码，默认为空
    opt.output_file = DFL_OUTPUT_FILENAME;     // 证书输出文件，默认为cert.crt
    opt.subject_name = DFL_SUBJECT_NAME;       // 证书使用者名称，默认为"CN=Cert, O=mbed TLS,C=UK"
    opt.issuer_name = DFL_ISSUER_NAME;         // 颁发者名称，默认为"CN=CA,O=mbed TLS,C=UK
    opt.not_before = DFL_NOT_BEFORE;           // 有效期开始时间，默认"20010101000000"
    opt.not_after = DFL_NOT_AFTER;             // 有效期结束时间，默认为"20301231235959"
    opt.serial = DFL_SERIAL;                   // 证书序列号，默认为1
    opt.selfsign = DFL_SELFSIGN;               // 是否是自签证书
    opt.is_ca = DFL_IS_CA;                     // 是否是CA证书
    opt.max_pathlen = DFL_MAX_PATHLEN;         // 证书链路径最大长度
    opt.key_usage = DFL_KEY_USAGE;             // 密钥用途，默认为空
    opt.ns_cert_type = DFL_NS_CERT_TYPE;       // 证书类型，默认为空
    opt.version = DFL_VERSION - 1;             // 证书版本，默认3
    opt.md = DFL_DIGEST;                       // hash 算法
    opt.subject_identifier = DFL_SUBJ_IDENT;   // add authority identifier to CRT
    opt.authority_identifier = DFL_AUTH_IDENT; // add subject identifier to CRT
    opt.basic_constraints = DFL_CONSTRAINTS;   // add basic constraints ext to CRT
    ...
    if (!opt.selfsign && strlen(opt.issuer_crt)) {
        ...
        ret = mbedtls_x509_dn_gets(issuer_name, sizeof(issuer_name), &issuer_crt.MBEDTLS_PRIVATE(subject));
        if (ret < 0) {
            mbedtls_strerror(ret, buf, 1024);
    ...
    if (!opt.selfsign && strlen(opt.request_file)) {
        ...
        ret = mbedtls_x509_dn_gets(subject_name, sizeof(subject_name), &csr.MBEDTLS_PRIVATE(subject));
        if (ret < 0) {
            mbedtls_strerror(ret, buf, 1024);
            mbedtls_printf(" failed\n  !  mbedtls_x509_dn_gets returned -0x%04x - %s\n\n", (unsigned int)-ret, buf);
            goto exit;
    ...
    if (!opt.selfsign && !strlen(opt.request_file)) {
        ...
        ret = mbedtls_pk_parse_keyfile(&loaded_subject_key, opt.subject_key, opt.subject_pwd, mbedtls_ctr_drbg_random, &ctr_drbg);
        if (ret != 0) {
            mbedtls_strerror(ret, buf, 1024);
    ...
    if (strlen(opt.issuer_crt)) {
        if (mbedtls_pk_check_pair(&issuer_crt.MBEDTLS_PRIVATE(pk), issuer_key, mbedtls_ctr_drbg_random, &ctr_drbg) != 0) {
    ...
    if (opt.version == MBEDTLS_X509_CRT_VERSION_3 && opt.authority_identifier != 0) {
        ...
        if (ret != 0) {
            mbedtls_strerror(ret, buf, 1024);
            mbedtls_printf(" failed\n  !  mbedtls_x509write_crt_set_authority_key_identifier returned -0x%04x - %s\n\n", (unsigned int)-ret, buf);
            goto exit;
        }
