...
int write_certificate_request(mbedtls_x509write_csr *req, const char *output_file, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    int ret;
    FILE *f;
...
int main(int argc, char *argv[])
{
    ...
    ret = mbedtls_pk_parse_keyfile(&key, opt.filename, opt.password, mbedtls_ctr_drbg_random, &ctr_drbg);
